...
//  Get4kVoca
//
//  Created by Oanh tran on 11/29/18.
...
protocol KeyboardAvoidable: class {
    /*
     Provide array of constraints, which should be adjusted to keyboard height value on its appear. After keyboard hidding, initial constants values will be applied.
     */
...
var ConstraintsDictionaryKey: UInt8 = 3
...
extension KeyboardAvoidable where Self: UIViewController {
    ...
    private func addKeyboardObservers(forConstraints constraints: [NSLayoutConstraint]?, customBlock block: ((CGFloat) -> Void)?) {
        removeKeyboardObservers()
        if let constraints = constraints {
            constraintsAndValues = constraints.reduce([NSLayoutConstraint: CGFloat](), { (result, constraint) -> [NSLayoutConstraint: CGFloat] in
                var result = result
        ...
        keyboardShowObserverObject = NotificationCenter.default.addObserver(forName: UIResponder.keyboardWillShowNotification, object: nil, queue: nil) { [weak self] notification in
            guard let notificationParameters = self?.getKeyboardShowParameters(fromNotification: notification) else {
                DDLogError("Cant extract keyboard notification parameters")
                return
            }
        ...
        keyboardHideObserverObject = NotificationCenter.default.addObserver(forName: UIResponder.keyboardWillHideNotification, object: nil, queue: nil) { [weak self] notification in
            guard let notificationParameters = self?.getKeyboardShowParameters(fromNotification: notification) else {
                DDLogError("Cant extract keyboard notification parameters")
                return
            }
    ...
    typealias KeyboardNotificationParameters = (height: CGFloat, duration: TimeInterval, animationOptions: UIView.AnimationOptions)
    ...
    private func animateLayout(parameters: KeyboardNotificationParameters, animations: @escaping () -> ()) {
